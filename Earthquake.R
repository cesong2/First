#setwd('C:/Users/tjoeun/Desktop/01_code/earthquake')
load("earthquake_16_21.rdata")
library(shiny)
library(leaflet)
library(ggplot2)
library(ggpmisc)
...
server <- function(input, output, session){
  filteredData <- reactive({
    quakes[quakes$mag>=input$range[1] & quakes$mag <=input$range[2]&quakes$year
           >= input$time[1]& quakes$year <= input$time[2],]
    })
  ...
  output$histCentile <-renderPlot({
    if (nrow(filteredData())==0)
        return(NULL)
        centileBreaks <- hist(plot = FALSE,filteredData()$mag, braks = 20)$breaks
        hist(filteredData()$mag,
             breaks = centileBreaks,
             main = "지진 발생 정보",xlab = "진도", ylab = "빈도", col='blue', border = 'grey')
  })
  output$depth <-renderPlot({
    ...
    observe({
      leafletProxy("map", data= filteredData()) %>% clearShapes() %>% 
        addCircles(radius =~log((quakes$mag))^20,
